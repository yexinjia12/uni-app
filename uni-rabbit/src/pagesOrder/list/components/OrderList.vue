...
import { ref } from 'vue'
import type { OrderListParams, OrderItem } from '@/types/order.d';
...
// 请求参数
const queryProps: OrderListParams = {
  page: 1,
  pageSize: 5,
...
const getMemberOrderData = async () => {
  const res = await getMemberOrderAPI(queryProps)
...
<template>
  <scroll-view scroll-y class="orders">
    <view class="card" v-for="item in orderList" :key="item.id">
      ...
      <navigator v-for="sku in item.skus" :key="sku.id" class="goods" :url="`/pagesOrder/detail/index?id=${item.id}`"
        hover-class="none">
        <view class="cover">
