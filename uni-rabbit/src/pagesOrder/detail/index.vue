...
// 获取当前页实例，数组最后一项
const pagesInstance = pages.at(-1) as any
// 页面渲染完成，滚动页面，添加滚动驱动的动画
onReady(() => {
  // 动画效果,导航栏背景色
  ...
  // 动画效果,导航栏标题
  pagesInstance.animate('.navbar .title', [{ color: 'transparent' }, { color: '#000' }], 1000, {
    scrollSource: '#scroller',
    timeRange: 1000,
    startScrollOffset: 0,
    endScrollOffset: 50,
  ...
  // 动画效果,导航栏返回按钮
  pagesInstance.animate('.navbar .back', [{ color: '#fff' }, { color: '#000' }], 1000, {
    scrollSource: '#scroller',
    timeRange: 1000,
    startScrollOffset: 0,
    endScrollOffset: 50,
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower">
    <template v-if="order">
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <!-- 其他订单状态:按需展示按钮 -->
        <template v-else>
          <navigator class="button secondary" :url="`/pagesOrder/create/index?orderId=${query.id}`" hover-class="none">
            再次购买
          </navigator>
          <!-- 待收货状态: 展示确认收货 -->
          <view v-if="order?.orderState == OrderState.DaiShouHuo" @tap="onOrderComfirm" class="button primary">
          ...
          <view v-if="[OrderState.DaiPingJia, OrderState.YiWanCheng, OrderState.YiQuXiao].includes(
    order.orderState,
  )
    " @tap="onOrderDelete" class="button delete">删除订单</view>
        </template>
      </view>
    </template>
    <template v-else>
